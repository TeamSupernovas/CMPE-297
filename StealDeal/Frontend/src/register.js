...
function Register({ onRegisterSuccess, prepopulatedEmail }) {
  ...
  const handleVerify2FA = async () => {
    try {
      const verifyResponse = await axios.post('http://localhost:8098/auth/validate_2fa', {
  ...
  const fetchQRCode = async (email) => {
    try {
      const response = await axios.get(`http://localhost:8098/auth/qr_code?email=${encodeURIComponent(email)}`);
